...
const METRIC_FIELD_MAP: Record<string, string[]> = {
  ...
  available_slots: ["available_slots", "Available Slots", "available slots", "available_slot", "Available Slot", "slots_available", "Slots Available", "availableSlots", "slots"],
  utilized_slots: ["utilized_slots", "Utilized Slots", "utilized slots", "utilized_slot", "Utilized Slot", "slots_utilized", "Slots Utilized", "utilizedSlots"],
...
const TOOLS = [
  { type: "function", name: "find_metric_definition", description: "Find the metric definition, formula, tags, and metadata for a KPI or metric query.", parameters: { type: "object", properties: { metric_query: { type: "string", description: "The KPI or metric to look up, such as conversion rate, AOV, leads, or gross margin." } }, required: ["metric_query"], additionalProperties: false } },
  { type: "function", name: "find_dashboard_link", description: "Find the best matching dashboard or report link.", parameters: { type: "object", properties: { dashboard_query: { type: "string", description: "Dashboard name such as ops dashboard, marketing dashboard, supply demand dashboard." } }, required: ["dashboard_query"], additionalProperties: false } },
  { type: "function", name: "find_dataset_link", description: "Find the best matching raw dataset link.", parameters: { type: "object", properties: { dataset_query: { type: "string", description: "Dataset name such as supply demand daily, channel market KPI daily, metric definitions." } }, required: ["dataset_query"], additionalProperties: false } },
  { type: "function", name: "business_question_drivers", description: "Identify the KPI in a business question, return its candidate drivers, and resolve the relevant datasets to analyze.", parameters: { type: "object", properties: { business_question: { type: "string", description: "A business question such as 'Why did conversion rate drop last week?'" } }, required: ["business_question"], additionalProperties: false } },
  { type: "function", name: "analyze_business_question", description: "Analyze a business question by resolving the KPI, relevant datasets, and likely drivers from linked data.", parameters: { type: "object", properties: { business_question: { type: "string", description: "A business question such as 'Why did conversion rate drop last week?'" } }, required: ["business_question"], additionalProperties: false } },
  { type: "function", name: "query_metric_value", description: "Return the actual metric value for a specific scope such as market, channel, and time period.", parameters: { type: "object", properties: { question: { type: "string", description: "A direct factual metric lookup question such as 'What is Dallas revenue in December 2025?'" } }, required: ["question"], additionalProperties: false } },
  { type: "function", name: "analyze_market_performance", description: "Rank markets for a KPI and identify which markets are underperforming versus the prior period.", parameters: { type: "object", properties: { business_question: { type: "string", description: "A market performance question such as 'Which markets are underperforming?'" } }, required: ["business_question"], additionalProperties: false } },
  { type: "function", name: "analyze_mix_change", description: "Analyze mix change over time, such as lead mix by channel or market this week versus last week.", parameters: { type: "object", properties: { business_question: { type: "string", description: "A mix change question such as 'What changed in lead mix this week?'" } }, required: ["business_question"], additionalProperties: false } },
  { type: "function", name: "analyze_metric_trend", description: "Analyze trend across the last several periods for a metric, with pacing-aware projection for the latest open period.", parameters: { type: "object", properties: { business_question: { type: "string", description: "A trend question such as 'monthly revenue trend' or 'trend for completed jobs this week'." } }, required: ["business_question"], additionalProperties: false } },
  { type: "function", name: "analyze_contribution_to_change", description: "Decompose the change in an additive metric by market or channel contribution.", parameters: { type: "object", properties: { business_question: { type: "string", description: "A question such as 'Which markets drove the revenue decline this month?'" } }, required: ["business_question"], additionalProperties: false } },
  { type: "function", name: "compare_segments", description: "Compare two markets or two channels for a metric in a selected time period.", parameters: { type: "object", properties: { business_question: { type: "string", description: "A comparison question such as 'Compare Atlanta vs Dallas completed jobs this month'." } }, required: ["business_question"], additionalProperties: false } },
] as const;
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    try {
      ...
      return jsonResponse({ error: msg, stack: error instanceof Error ? error.stack : null }, 500);
    }
  },
};
...
async function handleRequest(request: Request, env: Env): Promise<Response> {
  ...
  const body = (await request.json()) as { message?: string; prompt?: string; question?: string; input?: string };
  const userMessage = String(body?.message ?? body?.prompt ?? body?.question ?? body?.input ?? "").trim();

  if (!userMessage) {
    return jsonResponse({ error: "Missing message. Expected one of: message, prompt, question, input" }, 400);
  }

  if (normalize(userMessage) === "ping") {
    return jsonResponse({ answer: "pong", dataset: null, rows: [], dataset_link: null, dashboard_link: null });
  }
  ...
  const looksLikeBusinessQuestion = hasWord("why") || hasWord("driver") || hasWord("drivers") || hasWord("down") || hasWord("up") || hasWord("drop") || hasWord("dropped") || hasWord("increase") || hasWord("decrease") || hasWord("changed");
  const looksLikeTrendQuestion = hasWord("trend") || hasPhrase("month over month") || hasPhrase("week over week") || hasPhrase("monthly trend") || hasPhrase("weekly trend");
  const looksLikeContributionQuestion = hasPhrase("contribution to change") || hasPhrase("contributed most") || hasPhrase("drove the decline") || hasPhrase("drove the increase") || hasPhrase("which market drove") || hasPhrase("which channel drove");
  const looksLikeSegmentCompareQuestion = hasWord("compare") || hasWord("versus") || hasWord("vs");
  const looksLikeMarketPerformanceQuestion = hasPhrase("which markets are underperforming") || hasPhrase("underperforming markets") || hasPhrase("which markets underperform") || (hasWord("markets") && (hasWord("underperforming") || hasWord("underperform") || hasWord("worst")));
  const looksLikeMixChangeQuestion = hasPhrase("lead mix") || hasPhrase("channel mix") || hasPhrase("market mix") || (hasWord("mix") && (hasWord("changed") || hasWord("shift") || hasWord("shifted")));
  const looksLikeDashboardLinkQuestion = hasWord("dashboard") || hasWord("report") || hasPhrase("where can i find") || hasPhrase("where is");
  const looksLikeDatasetLinkQuestion = hasWord("dataset") || hasWord("json") || hasWord("sheet") || hasPhrase("raw data") || hasPhrase("which dataset") || hasPhrase("what dataset") || hasPhrase("should i use") || hasPhrase("use for") || hasPhrase("best dataset");
  const looksLikeMetricValueQuestion = !looksLikeBusinessQuestion && !looksLikeMarketPerformanceQuestion && !looksLikeMixChangeQuestion && !looksLikeTrendQuestion && !looksLikeContributionQuestion && !looksLikeSegmentCompareQuestion && (hasPhrase("what is") || hasPhrase("whats") || hasPhrase("what was") || hasPhrase("how much") || hasPhrase("show me") || hasPhrase("give me")) && (hasWord("revenue") || hasWord("aov") || hasWord("leads") || hasWord("conversion") || hasPhrase("conversion rate") || hasPhrase("booking rate") || hasPhrase("cancel rate") || hasWord("ctr") || hasWord("cpc") || hasPhrase("marketing spend") || hasPhrase("jobs completed") || hasPhrase("jobs booked") || hasPhrase("available slot") || hasPhrase("available slots"));

  const directDatasetMatch = await tryDirectDatasetShortcut(userMessage);

  if (directDatasetMatch && looksLikeDatasetLinkQuestion) {
    return jsonResponse(buildAppResponse({ answer: `You should use this dataset: ${directDatasetMatch.link}`, dataset: directDatasetMatch.dataset || directDatasetMatch.sheet_name || "dataset_list", datasetLink: directDatasetMatch.link || null, rows: [directDatasetMatch], data: { found: true, dataset: directDatasetMatch } }));
  }

  if (looksLikeDatasetLinkQuestion) {
    const result = await findDatasetLink(userMessage);
    return jsonResponse(result.found ? buildAppResponse({ answer: `You should use this dataset: ${result.dataset.link}`, dataset: result.dataset.dataset || result.dataset.sheet_name || "dataset_list", datasetLink: result.dataset.link || null, rows: [result.dataset], data: result }) : buildAppResponse({ answer: result.message, dataset: null, rows: [], data: result }));
  }

  if (looksLikeDashboardLinkQuestion) {
    const result = await findDashboardLink(userMessage);
    return jsonResponse(result.found ? buildAppResponse({ answer: `You can find the dashboard here: ${result.dashboard.url}`, dataset: "dashboard_links", dashboardLink: result.dashboard.url || null, rows: [result.dashboard], data: result }) : buildAppResponse({ answer: result.message, dataset: null, rows: [], data: result }));
  }

  if (looksLikeTrendQuestion) {
    const trendResult = await analyzeMetricTrend(userMessage);
    return jsonResponse(trendResult.found ? buildAppResponse({ answer: stripMarkdownBold([trendResult.analysis.summary, ...trendResult.analysis.observations.slice(0, 6)].join("\n")), dataset: trendResult.datasets_used[0]?.dataset || trendResult.metric.metric_id, datasetLink: trendResult.datasets_used[0]?.link || null, rows: trendResult.analysis.points.map((p) => ({ period: p.period, value: p.value, raw_value: p.raw_value, is_projected: p.is_projected })), data: trendResult }) : buildAppResponse({ answer: trendResult.message, dataset: null, rows: [], data: trendResult }));
  }

  if (looksLikeContributionQuestion) {
    const contributionResult = await analyzeContributionToChange(userMessage);
    return jsonResponse(contributionResult.found ? buildAppResponse({ answer: stripMarkdownBold([contributionResult.analysis.summary, ...contributionResult.analysis.observations.slice(0, 6)].join("\n")), dataset: contributionResult.datasets_used[0]?.dataset || contributionResult.metric.metric_id, datasetLink: contributionResult.datasets_used[0]?.link || null, rows: contributionResult.analysis.contributions.map((r) => ({ segment: r.segment, current_value: r.current_value, prior_value: r.prior_value, contribution_to_change: r.contribution_to_change })), data: contributionResult }) : buildAppResponse({ answer: contributionResult.message, dataset: null, rows: [], data: contributionResult }));
  }

  if (looksLikeSegmentCompareQuestion) {
    const compareResult = await compareSegments(userMessage);
    if (compareResult.found) {
      return jsonResponse(buildAppResponse({ answer: compareResult.comparison.summary, dataset: compareResult.datasets_used[0]?.dataset || compareResult.metric.metric_id, datasetLink: compareResult.datasets_used[0]?.link || null, rows: [{ segment: compareResult.comparison.segment_a.name, value: compareResult.comparison.segment_a.value, raw_value: compareResult.comparison.segment_a.raw_value }, { segment: compareResult.comparison.segment_b.name, value: compareResult.comparison.segment_b.value, raw_value: compareResult.comparison.segment_b.raw_value }], data: compareResult }));
    }
  }

  if (looksLikeMarketPerformanceQuestion) {
    const marketResult = await analyzeMarketPerformance(userMessage);
    return jsonResponse(marketResult.found ? buildAppResponse({ answer: stripMarkdownBold([marketResult.analysis.summary, ...marketResult.analysis.observations.slice(0, 6)].join("\n")), dataset: marketResult.datasets_used[0]?.dataset || marketResult.metric.metric_id, datasetLink: marketResult.datasets_used[0]?.link || null, rows: marketResult.analysis.underperforming_markets.map((m) => ({ market: m.market, current_value: m.current_value, prior_value: m.prior_value, delta_value: m.delta_value })), data: marketResult }) : buildAppResponse({ answer: stripMarkdownBold(marketResult.message), dataset: null, rows: [], data: marketResult }));
  }

  if (looksLikeMixChangeQuestion) {
    const mixResult = await analyzeMixChange(userMessage);
    return jsonResponse(mixResult.found ? buildAppResponse({ answer: stripMarkdownBold([mixResult.analysis.summary, ...mixResult.analysis.observations.slice(0, 6)].join("\n")), dataset: mixResult.datasets_used[0]?.dataset || "mix_analysis", datasetLink: mixResult.datasets_used[0]?.link || null, rows: mixResult.analysis.changes.map((c) => ({ dimension_value: c.dimension_value, current_share: c.current_share, prior_share: c.prior_share, share_delta: c.share_delta, current_value: c.current_value, prior_value: c.prior_value })), data: mixResult }) : buildAppResponse({ answer: stripMarkdownBold(mixResult.message), dataset: null, rows: [], data: mixResult }));
  }

  if (looksLikeMetricValueQuestion) {
    const valueResult = await queryMetricValue(userMessage);
    return jsonResponse(valueResult.found ? buildAppResponse({ answer: stripMarkdownBold(valueResult.result.summary), dataset: valueResult.datasets_used[0]?.dataset || valueResult.metric.metric_id, datasetLink: valueResult.datasets_used[0]?.link || null, rows: valueResult.datasets_used.map((d) => ({ dataset: d.dataset, dataset_link: d.link || null, row_count: d.row_count })), data: valueResult }) : buildAppResponse({ answer: stripMarkdownBold(valueResult.message), dataset: null, rows: [], data: valueResult }));
  }

  if (looksLikeBusinessQuestion) {
    const directAnalysis = await analyzeBusinessQuestion(userMessage);
    return jsonResponse(directAnalysis.found ? buildAppResponse({ answer: stripMarkdownBold([directAnalysis.analysis.summary, ...directAnalysis.analysis.observations.slice(0, 6)].join("\n")), dataset: directAnalysis.datasets_used[0]?.dataset || directAnalysis.metric.metric_id, datasetLink: directAnalysis.datasets_used[0]?.link || null, rows: directAnalysis.datasets_used.map((d) => ({ dataset: d.dataset, dataset_link: d.link || null, row_count: d.row_count, current_row_count: d.current_row_count ?? null, prior_row_count: d.prior_row_count ?? null })), data: directAnalysis }) : buildAppResponse({ answer: stripMarkdownBold(directAnalysis.message), dataset: null, rows: [], data: directAnalysis }));
  }

  const firstResp = await callOpenAI(env.OPENAI_API_KEY, {
    model: "gpt-5.4-mini",
    input: [
      { role: "system", content: [{ type: "input_text", text: "You are a NuBrakes business analyst assistant. Use tools when needed. Use find_metric_definition when the user asks what a KPI means. Use find_dashboard_link when the user asks for a dashboard or report link. Use find_dataset_link when the user asks for a dataset or raw data link. Use business_question_drivers when the user asks why a KPI moved or what drove performance. Use analyze_business_question when the user asks for an actual driver analysis from linked data. Use query_metric_value when the user asks for a direct metric value for a market/channel/time period. Use analyze_market_performance for questions about which markets are underperforming or worst. Use analyze_mix_change for questions about lead mix, channel mix, or market mix changes. Use analyze_metric_trend for trend questions. Use analyze_contribution_to_change for decomposition questions. Use compare_segments for vs/compare questions. Keep answers concise, quantitative when possible, and business-focused. Do not use markdown bold. Never use double asterisks in the response. Return plain text only." }] },
  ...
  if (toolOutputs.length === 0) {
    return jsonResponse(buildAppResponse({ answer: stripMarkdownBold(extractResponseText(firstResp) || "No response generated."), dataset: null, rows: [], debug: firstResp }));
  }
  ...
  return jsonResponse(mergeStructuredToolResultIntoResponse(stripMarkdownBold(extractResponseText(secondResp) || "No response generated."), lastStructuredResult, secondResp));
}

function buildAppResponse(args: { answer: string; dataset?: string | null; rows?: Array<Record<string, unknown>>; datasetLink?: string | null; dashboardLink?: string | null; data?: unknown; debug?: unknown }): AppApiResponse {
  return { answer: stripMarkdownBold(args.answer), dataset: args.dataset ?? null, rows: args.rows ?? [], dataset_link: args.datasetLink ?? null, dashboard_link: args.dashboardLink ?? null, data: args.data, debug: args.debug };
}

function mergeStructuredToolResultIntoResponse(answer: string, toolResult: unknown, debug?: unknown): AppApiResponse {
  const base = buildAppResponse({ answer: stripMarkdownBold(answer), dataset: null, rows: [], datasetLink: null, dashboardLink: null, data: toolResult, debug });
  ...
  if ("dataset" in r) {
    ...
    return buildAppResponse({ answer: stripMarkdownBold(answer), dataset: dataset?.dataset || dataset?.sheet_name || null, rows: dataset ? [dataset as Record<string, unknown>] : [], datasetLink: dataset?.link || null, dashboardLink: null, data: toolResult, debug });
  }

  if ("dashboard" in r) {
    const dashboard = r.dashboard as DashboardDefinition;
    return buildAppResponse({ answer: stripMarkdownBold(answer), dataset: "dashboard_links", rows: dashboard ? [dashboard as Record<string, unknown>] : [], datasetLink: null, dashboardLink: dashboard?.url || null, data: toolResult, debug });
  }

  if ("datasets_used" in r) {
    const structured = r as { datasets_used?: Array<{ dataset: string; link?: string; row_count: number; current_row_count?: number; prior_row_count?: number }>; metric?: MetricDefinition };
    const datasetsUsed = structured.datasets_used || [];
    return buildAppResponse({ answer: stripMarkdownBold(answer), dataset: datasetsUsed[0]?.dataset || structured.metric?.metric_id || null, rows: datasetsUsed.map((d) => ({ dataset: d.dataset, dataset_link: d.link || null, row_count: d.row_count, current_row_count: d.current_row_count ?? null, prior_row_count: d.prior_row_count ?? null })), datasetLink: datasetsUsed[0]?.link || null, dashboardLink: null, data: toolResult, debug });
  }
...
async function handleToolCall(name: string, args: Record<string, unknown>) {
  switch (name) {
    case "find_metric_definition": return await findMetricDefinition(String(args.metric_query || ""));
    case "find_dashboard_link": return await findDashboardLink(String(args.dashboard_query || ""));
    case "find_dataset_link": return await findDatasetLink(String(args.dataset_query || ""));
    case "business_question_drivers": return await buildBusinessQuestionDriverPlan(String(args.business_question || ""));
    case "analyze_business_question": return await analyzeBusinessQuestion(String(args.business_question || ""));
    case "query_metric_value": return await queryMetricValue(String(args.question || ""));
    case "analyze_market_performance": return await analyzeMarketPerformance(String(args.business_question || ""));
    case "analyze_mix_change": return await analyzeMixChange(String(args.business_question || ""));
    case "analyze_metric_trend": return await analyzeMetricTrend(String(args.business_question || ""));
    case "analyze_contribution_to_change": return await analyzeContributionToChange(String(args.business_question || ""));
    case "compare_segments": return await compareSegments(String(args.business_question || ""));
    default: return { error: `Unknown tool: ${name}` };
  }
}

async function fetchJsonCached<T = unknown>(url: string, ttlMs = CACHE_TTL_MS): Promise<T> {
  const now = Date.now();
  ...
  const promise = (async () => {
    ...
    const res = await fetch(url, { method: "GET", headers: { "Cache-Control": `public, max-age=${Math.floor(ttlMs / 1000)}` }, cf: { cacheTtl: Math.floor(ttlMs / 1000), cacheEverything: true } });
    ...
    await cache.put(cacheKey, new Response(JSON.stringify(value), { headers: { "Content-Type": "application/json", "Cache-Control": `public, max-age=${Math.floor(ttlMs / 1000)}` } }));
    return value;
  })();
...
function normalizeWorkerRow(row: DatasetRow): DatasetRow {
  const day = row["day"] ?? row["Day"] ?? row["date"] ?? row["Date"] ?? row["week"] ?? row["Week"] ?? row["month"] ?? row["Month"] ?? "";
  const week = row["week"] ?? row["Week"] ?? row["day"] ?? row["Day"] ?? row["date"] ?? row["Date"] ?? row["month"] ?? row["Month"] ?? "";
  const month = row["month"] ?? row["Month"] ?? row["day"] ?? row["Day"] ?? row["date"] ?? row["Date"] ?? "";
  const date = row["date"] ?? row["Date"] ?? row["day"] ?? row["Day"] ?? row["month"] ?? row["Month"] ?? "";
  return { ...row, day, Day: day, week, Week: week, month, Month: month, date, Date: date };
}

async function loadJsonFromUrl<T = unknown>(url: string): Promise<T> {
  const data = await fetchJsonCached<T>(url, DATASET_TTL_MS);
  if (Array.isArray(data)) {
    return data.map((row) => row && typeof row === "object" && !Array.isArray(row) ? normalizeWorkerRow(row as DatasetRow) : row) as T;
  }
  return data;
}
...
async function loadMetricRegistry(): Promise<Map<string, MetricRegistryEntry>> {
  ...
  if (mem && mem.expiresAt > Date.now()) return mem.value as Map<string, MetricRegistryEntry>;
  ...
  memoryCache.set(cacheKey, { value: map, expiresAt: Date.now() + CACHE_TTL_MS });
  return map;
}

function buildConfidenceScore(args: { metricMatched?: boolean; metricScore?: number; datasetCount?: number; currentRows?: number; priorRows?: number; usableDrivers?: number; totalDrivers?: number; explicitScope?: boolean }): ConfidenceScore {
  let score = 0;
  ...
  const label: ConfidenceLabel = normalized >= 75 ? "high" : normalized >= 45 ? "medium" : "low";
...
async function tryDirectDatasetShortcut(userMessage: string): Promise<DatasetDefinition | null> {
  ...
  const shortcutMatchers: Array<{ patterns: string[]; sheetName: string }> = [
    { patterns: ["supply and demand by market", "supply demand by market", "supply and demand", "supply demand", "market level supply demand"], sheetName: "fact nubrakes supply demand daily" },
    { patterns: ["channel market kpi", "leads booked completed revenue by market and channel", "conversion by market and channel"], sheetName: "fact nubrakes channel market kpi daily" },
    { patterns: ["marketing performance", "impressions clicks ctr marketing spend"], sheetName: "fact nubrakes marketing performance daily" },
  ];

  for (const shortcut of shortcutMatchers) {
    if (shortcut.patterns.some((p) => q.includes(normalize(p)))) {
      const match = datasets.find((d) => normalize(String(d.sheet_name || "")) === shortcut.sheetName);
...
function scoreDashboardEntry(query: string, entry: DashboardDefinition): number {
  ...
  const STOPWORDS = new Set(["a", "an", "the", "for", "of", "to", "in", "on", "by", "and", "or", "with", "from", "link", "need", "show", "give", "me", "where", "can", "i", "find"]);
...
function scoreDatasetEntry(query: string, entry: DatasetDefinition): number {
  ...
  const STOPWORDS = new Set(["a", "an", "the", "for", "of", "to", "in", "on", "by", "and", "or", "with", "from", "data", "dataset", "json", "link", "file", "need", "show", "give", "me", "where", "can", "i", "find", "should", "use", "best"]);
  const queryWords = q.split(/\s+/).map((w) => w.trim()).filter((w) => w && !STOPWORDS.has(w));
...
async function findDashboardLink(dashboardQuery: string): Promise<DashboardLookupResult> {
  ...
  if (!bestMatch || bestScore < 20) return { found: false, message: `No confident dashboard link found for query: ${dashboardQuery}` };
  return { found: true, dashboard: bestMatch, confidence: buildConfidenceScore({ metricMatched: true, metricScore: bestScore, datasetCount: 1, explicitScope: true }) };
}
...
async function findDatasetLink(datasetQuery: string): Promise<DatasetLookupResult> {
  ...
  if (!bestMatch || bestScore < 20) return { found: false, message: `No confident dataset link found for query: ${datasetQuery}` };
  return { found: true, dataset: bestMatch, confidence: buildConfidenceScore({ metricMatched: true, metricScore: bestScore, datasetCount: 1, explicitScope: true }) };
}
...
async function findMetricDefinition(metricQuery: string): Promise<MetricLookupResult> {
  ...
  if (!q) return { found: false, message: "Metric query is empty" };

  const STOPWORDS = new Set(["a", "an", "the", "for", "of", "to", "in", "on", "by", "and", "or", "with", "from", "what", "why", "how", "did", "does", "is", "are", "was", "were", "show", "tell", "me", "about", "metric", "kpi", "driver", "drivers", "business", "question", "drop", "dropped", "increase", "increased", "decrease", "decreased", "change", "changed", "trend", "performing", "performance", "last", "this", "week", "month", "day", "during", "compare", "vs", "versus"]);
  const queryWords = q.split(/\s+/).map((w) => w.trim()).filter((w) => w && !STOPWORDS.has(w));
  ...
  if (!bestMatch || bestScore < 25) return { found: false, message: `No metric definition found for query: ${metricQuery}` };
  return { found: true, metric: bestMatch, score: bestScore, confidence: buildConfidenceScore({ metricMatched: true, metricScore: bestScore, explicitScope: true }) };
}
...
async function buildBusinessQuestionDriverPlan(businessQuestion: string): Promise<BusinessQuestionDriverResult> {
  ...
  return {
    ...
    confidence: buildConfidenceScore({ metricMatched: true, metricScore: metricResult.score, datasetCount: datasets.length, totalDrivers: candidateDrivers.length, usableDrivers: driverDefinitions.length, explicitScope: true }),
  };
}

/* Full remaining analysis functions continue unchanged except for pacing helpers below.
   To keep this usable in-chat, the fully patched worker is in the canvas textdoc.
   The critical runtime changes are already included there: row normalization, bucket fallback, and updated pacing logic. */
...
function parseBusinessQuestionScopeFromRows(businessQuestion: string, rows: DatasetRow[]): ParsedBusinessScope {
  ...
  if (q.includes("today") || q.includes("yesterday") || q.includes("daily")) {
    time_grain = "day";
  } else if (q.includes("month") || q.includes("monthly") || q.includes("mtd") || MONTH_NAMES.some((m) => q.includes(m))) {
    time_grain = "month";
  }
...
function parsePointInTimeScopeFromRows(question: string, rows: DatasetRow[]): ParsedPointInTimeScope {
  ...
  if (monthName) {
    ...
    if (explicitYear) yearToUse = explicitYear;
    else {
      const yearsWithMonth = availableYears.filter((y) => rows.some((row) => {
    ...
    if (latest) { target_bucket = latest; period_label = latest; time_grain = "month"; }
  } else if (q.includes("last month")) {
    const latestAndPrior = getLatestAndPriorBucket(rows, "month");
    if (latestAndPrior.prior) { target_bucket = latestAndPrior.prior; period_label = latestAndPrior.prior; time_grain = "month"; }
  } else if (q.includes("this week") || q.includes("weekly trend")) {
    const latest = getLatestBucket(rows, "week");
    if (latest) { target_bucket = latest; period_label = latest; time_grain = "week"; }
  } else if (q.includes("last week")) {
    const latestAndPrior = getLatestAndPriorBucket(rows, "week");
    if (latestAndPrior.prior) { target_bucket = latestAndPrior.prior; period_label = latestAndPrior.prior; time_grain = "week"; }
  } else {
    const latest = getLatestBucket(rows, time_grain);
    if (latest) { target_bucket = latest; period_label = latest; }
  }
...
function extractAvailableYears(rows: DatasetRow[], fields: string[]): number[] {
  ...
  for (const row of rows) {
    for (const field of fields) {
      ...
      if (directYear) { years.add(Number(directYear[1])); continue; }
...
function findComparedSegments(normalizedQuestion: string, values: string[]): string[] {
  ...
  for (const raw of values) {
    ...
    for (const word of words) if (word.length >= 3 && normalizedQuestion.includes(word)) score += 10;
...
function splitRowsCurrentVsPrior(rows: DatasetRow[], grain: TimeGrain, targetBucket?: string): ScopedRows {
  ...
  if (!sortedKeys.length) return { current: [], prior: [], current_label: "current period", prior_label: "prior period" };
  ...
  return { current: bucketMap.get(currentKey) || [], prior: priorKey ? bucketMap.get(priorKey) || [] : [], current_label: currentKey, prior_label: priorKey || "previous period unavailable" };
}
...
function getBucketDateFieldCandidates(grain: TimeGrain): string[] {
  if (grain === "day") return ["day", "Day", "date", "Date"];
  if (grain === "week") return ["week", "Week", "day", "Day", "date", "Date"];
...
function canonicalFieldName(value: string): string {
  return String(value || "").replace(/([a-z])([A-Z])/g, "$1 $2").toLowerCase().trim().replace(/[_-]+/g, " ").replace(/\s+/g, " ");
}
...
function computeMetricValue(metricId: string, rows: DatasetRow[]): number | null {
  ...
  if (ADDITIVE_METRICS.has(id)) return directMetricSum ?? null;
  ...
  switch (id) {
    case "booking_rate": return safeDivide(jobsBooked, leads) ?? directMetricAvg;
    case "conversion_rate": return safeDivide(jobsCompleted, leads) ?? directMetricAvg;
    case "cancel_rate": return safeDivide(canceledJobs, jobsBooked) ?? directMetricAvg;
    case "cancel_outcome_rate": return safeDivide(canceledJobs, addNullable(jobsCompleted, canceledJobs)) ?? directMetricAvg;
    case "aov": return safeDivide(revenue, jobsCompleted) ?? directMetricAvg;
    case "ctr": return safeDivide(clicks, impressions) ?? directMetricAvg;
    case "cpc": return safeDivide(marketingSpend, clicks) ?? directMetricAvg;
    case "cost_per_inquiry": return safeDivide(marketingSpend, leads) ?? directMetricAvg;
    case "mac": return safeDivide(marketingSpend, jobsCompleted) ?? directMetricAvg;
    case "technician_utilization": return safeDivide(utilizedSlots, availableSlots) ?? directMetricAvg;
    case "customer_cancel_rate": return safeDivide(customerCancels, jobsBooked) ?? directMetricAvg;
    case "hq_cancel_rate": return safeDivide(hqCancels, jobsBooked) ?? directMetricAvg;
    case "reschedule_rate": return safeDivide(addNullable(customerReschedules, hqReschedules), jobsBooked) ?? directMetricAvg;
    case "customer_reschedule_rate": return safeDivide(customerReschedules, jobsBooked) ?? directMetricAvg;
    case "hq_reschedule_rate": return safeDivide(hqReschedules, jobsBooked) ?? directMetricAvg;
    default: return RATE_METRICS.has(id) ? directMetricAvg : directMetricSum;
  }
}
...
function getRowDateForPacing(row: DatasetRow): Date | null {
  const raw = row["day"] ?? row["Day"] ?? row["date"] ?? row["Date"] ?? row["week"] ?? row["Week"] ?? row["month"] ?? row["Month"];
  if (!raw) return null;
  const value = String(raw).trim();
...
function calcHistoricalPacingForWorker(grain: TimeGrain, rows: DatasetRow[], metricId: string, targetBucket?: string, lookbackDays = 90): { projected: number; actual: number; pct: number; method: "historical" | "fallback"; sampleSize: number } | null {
  console.log("=== pacing debug start ===", { grain, metricId, targetBucket, lookbackDays, rowCount: rows.length });
  ...
  for (const row of currentRows) {
    const d = getRowDateForPacing(row);
    if (!d) continue;
    if (!currentMaxDate || d.getTime() > currentMaxDate.getTime()) currentMaxDate = d;
  }
  ...
  const currentElapsedWeekdayCounts = countWeekdaysBetweenLocal(currentPeriodStart, currentMaxDate);
  const fullCurrentPeriodWeekdayCounts = countWeekdaysBetweenLocal(currentPeriodStart, currentPeriodEnd);
  ...
  for (const bucket of historicalKeys) {
    ...
    if (share && share > 0 && share <= 1.25) shares.push(share);
  }
  ...
  if (!shares.length) {
    ...
    const result = { actual: currentActual, projected: fallbackPct > 0 ? currentActual / fallbackPct : currentActual, pct: fallbackPct, method: "fallback" as const, sampleSize: 0 };
    console.log("pacing final", result);
    return result;
  }

  const historicalPct = shares.reduce((a, b) => a + b, 0) / shares.length;
  const result = { actual: currentActual, projected: historicalPct > 0 ? currentActual / historicalPct : currentActual, pct: historicalPct, method: "historical" as const, sampleSize: shares.length };
  console.log("pacing final", { ...result, historicalPct });
  return result;
}
...
function getDriverRelationship(metricId: string, driverId: string): DriverRelationship {
  ...
  const negativePairs = new Set(["jobs_completed|cancel_rate", "jobs_completed|customer_cancel_rate", "jobs_completed|hq_cancel_rate", "jobs_completed|reschedule_rate", "jobs_completed|customer_reschedule_rate", "jobs_completed|hq_reschedule_rate", "conversion_rate|cancel_rate", "conversion_rate|customer_cancel_rate", "conversion_rate|hq_cancel_rate", "conversion_rate|reschedule_rate", "booking_rate|cancel_rate", "booking_rate|customer_cancel_rate", "booking_rate|hq_cancel_rate", "revenue|cancel_rate", "revenue|customer_cancel_rate", "revenue|hq_cancel_rate"]);
  const contextualDrivers = new Set(["technician_utilization", "ft_tech_utilization", "pt_tech_utilization", "available_slots", "utilized_slots", "marketing_spend", "cpc", "cost_per_inquiry", "mac"]);
...
function rankDriverObservations(drivers: DriverObservation[]): DriverObservation[] {
  const bucketRank: Record<DriverObservation["explanatoryDirection"], number> = { hurts: 4, supports: 3, context: 2, mixed: 1, unknown: 0 };
...
function buildDriverObservationText(obs: DriverObservation, label: "headwind" | "tailwind" | "context", metricChangeDirection?: "up" | "down" | "flat" | "unknown"): string {
  if (obs.currentValue === null && obs.priorValue === null) return `${prettifyMetricLabel(obs.driverId)} is not directly computable from ${obs.datasetUsed}.`;
  ...
  if (label === "headwind") {
    if (metricChangeDirection === "up") return `${metricLabel} moved from ${prior} to ${current} (${delta}), which acted as a headwind against the increase.`;
    return `${metricLabel} moved from ${prior} to ${current} (${delta}), which looks like a headwind.`;
  }
  if (label === "tailwind") {
    if (metricChangeDirection === "down") return `${metricLabel} moved from ${prior} to ${current} (${delta}), which partially offset the decline.`;
    if (metricChangeDirection === "up") return `${metricLabel} moved from ${prior} to ${current} (${delta}), which contributed to the increase.`;
...
function buildAnalysisSummary(args: { metric: MetricDefinition; currentMetricValue: number | null; priorMetricValue: number | null; deltaMetricValue: number | null; rankedDrivers: DriverObservation[]; currentLabel: string; priorLabel: string }): string {
  ...
  if (currentMetricValue === null || priorMetricValue === null) return `${metric.metric_name} could not be fully computed for ${currentLabel} versus ${priorLabel}.`;
  ...
  if (metricDirection === "up") {
    if (primarySupport) summary += ` Primary contributing factor: ${prettifyMetricLabel(primarySupport.driverId)}.`;
    if (primaryHeadwind) summary += ` Headwind: ${prettifyMetricLabel(primaryHeadwind.driverId)}.`;
  } else if (metricDirection === "down") {
    if (primaryHeadwind) summary += ` Primary driver of the decline: ${prettifyMetricLabel(primaryHeadwind.driverId)}.`;
    if (primarySupport) summary += ` Partial offset: ${prettifyMetricLabel(primarySupport.driverId)}.`;
  } else {
    if (primarySupport) summary += ` Supporting factor: ${prettifyMetricLabel(primarySupport.driverId)}.`;
    if (primaryHeadwind) summary += ` Headwind: ${prettifyMetricLabel(primaryHeadwind.driverId)}.`;
  }
  return summary;
}
...
function formatDeltaValue(value: number | null, formatType?: string): string {
  ...
  const sign = value > 0 ? "+" : "";
  switch (normalize(formatType || "")) {
    case "percent": return `${sign}${(value * 100).toFixed(1)} pts`;
...
function inferFormatType(metricId: string): string {
  const id = canonicalMetricId(metricId);
  if (["booking_rate", "conversion_rate", "cancel_rate", "cancel_outcome_rate", "ctr", "technician_utilization", "ft_tech_utilization", "pt_tech_utilization", "gross_margin_pct", "parts_cost_pct_revenue", "labor_cost_pct_revenue", "marketing_spend_pct_revenue", "customer_cancel_rate", "hq_cancel_rate", "reschedule_rate", "customer_reschedule_rate", "hq_reschedule_rate"].includes(id)) return "percent";
  if (["revenue", "aov", "marketing_spend", "cpc", "cost_per_inquiry", "mac", "net_contribution_profit"].includes(id)) return "currency";
  return "number";
}
